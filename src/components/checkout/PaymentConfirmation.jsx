...
function PaymentConfirmation(){
    ...
    const serchParam = new URLSearchParams(location.search);
    const dispatch = useDispatch();
    const { cart } = useSelector((state) => state.carts);
    const [ errorMessage, setErrorMessage ] = useState("");
    const [ loading, setLoading ] = useState(false);

    const paymentIntent = serchParam.get("payment_intent");
    const clientSecret = serchParam.get("payment_intent_client_secret");
    const redirectStatus = serchParam.get("redirect_status");
